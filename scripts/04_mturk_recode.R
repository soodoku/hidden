...
# Read in the data
mturk <- read.csv("data/mturk/hidden_knowledge_March 28, 2017_20.29.csv", header = TRUE, stringsAsFactors = FALSE)
mturk <- mturk[-c(1:2),]

# Valid 
mturk$valid <- recode(mturk$consent, "I agree" = "1", .default = NA_character_)
# Take out invalid 
mturk <- subset(mturk, !is.na(valid) & mturk$ccode=="US")
# Take out one where randomization is an empty string 
mturk <- subset(mturk, cheat_no_cheat != "")
                                        ...
                                        # 1/3--2/3
mturk$cnc_test <- recode(mturk$cheat_no_cheat, "no_cheat" = "No Treatment", "cheat" = "Treatment", .missing = "0")
...
mturk$visual_test <- recode(mturk$visual, .missing = "0")
...
mturk$fugivisual <- recode(mturk$fugitive_visual, .missing = "0")
# Crosstab needs to show 50:50 randomization
...
#mturk$gg2_7   <- nona(as.numeric(mturk$rg_s_gg2_2) > 5)
